...
'''.split(): onlineVideos.append(url)
    ...
    #out = sys.stdout
    out = open(outname,'w')
    print('<!-- Do not edit this file. It is created by makeGitTutorial.py from info in tutorialIndex.py --!>',file=out)
...
''',file=out)
    ...
    for tutdir in glob.glob(os.path.join(TutorialsDir,'*')):
        ...
        if d in ['scripts','docs','webdocs','MDtutorials','MDhelp','Outtakes',
                     'selftestdata','help','tutorial_template']: continue
        if d not in dirList: print(u"Tutorial directory not in tutorialIndex: "+d)
