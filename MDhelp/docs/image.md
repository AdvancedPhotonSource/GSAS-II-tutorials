...
* E. The **Calibration controls** determine the process used to search for diffraction rings when the detector calibration is used. The "Show ring picks" check button determines if the located ring positions are displayed, but does not affect the computation. The choices in the Calibrant menu button come from file `ImageCalibrants.py` in the GSASII directory. If you want to add custom entries to that menu, create a file named `UserCalibrants.py` in the same directory; see the comments in `ImageCalibrants.py` as a guide for what to put in that file.
...
* Calibrate: The calibrate command will expect you to have already set a calibrant and as well as any other settings in the Calibration controls and then to click on several points for the first diffraction ring with the left mouse button. Then click on the right mouse button to begin the calibration process, which will search for the location of additonal rings.
* Recalibrate: Repeats the calibration process using the previously-located position for the first ring and current image settings.
* Recalibrate all: Repeats the calibration process using the previously-located position for the first ring and current image settings, but for a series of selected images. 
* Calculate rings: Computes and displays the ring positions from the calibration settings. Does not change any values. 
* Multi-distance Recalibrate: Performs a calibration on a series of images that have different distance placements. The tilt angle, the tilt rotation, the penetration and the wavelength, will be required to be the same for all included images, if refined. The beam center X & Y values are independent for each image. This command can operate in two modes: If any of the detector distance settings are set to be refined, then those distances are optimized individually (the wavelength cannot be refined in this case). If none of the distances are set to be refined, then a overall offset for all detectors is fit. The wavelength parameter will also be refined if the refine flag is on for the first image.
* Clear calibration: Deletes previous ring pick information from the current image
<!-- 
* Multiimage Gain map: TBD
-->
